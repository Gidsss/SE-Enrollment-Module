...
<div style="max-width:100%; height:50px; position: relative; border-bottom: .5px solid grey;">
   <br><br><br>
   <a href="" style="text-decoration: none;">
   <span style="color: black; position: absolute; top: 26%; right: 5%; font-family: Inter, sans-serif; font-size: 16px; cursor: pointer;">Home/Enrollment/Class Creation</span>
   </a>
</div>
         ...
         <div id="faculty-container">
            <section id="mainsection">
               <div class="add-div-class" style="position: relative; bottom: 180px; left: 56px; display: flex; ">
                  <input id="faculty-input-0" type="text" class="form-control faculty-input" style="width: 420px;" required placeholder="Select Faculty">
                  <div class="input-group-prepend">
                     <button type="button" class="btn btn-default" data-toggle="dropdown">
                     <i class="fas fa-caret-down"></i>
                     </button>
                  ...
                  <div class="card" style="position:absolute   ;background-color:#CDCDCD ; color: white; width:150ch;left:130px; height:50px;top:-25px">
                     ...
                     <div class="form-group">
                        <div class="input-group mb-3" style="position:absolute;width:200px; left:50px; top:6px">
                           <input id="selectday-input-0" type="text" class="form-control selectday" placeholder="Select Day" required>
                           <div class="input-group-append">
                              <button type="button" class="btn btn-default" data-toggle="dropdown">
                              <i class="fas fa-caret-down"></i>
                              </button>
...
<script>
   ...
   document.getElementById("newsectionbtn").onclick = function () {
      expandDiv();
       if (clickCounter < 7) {
           ...
           var facultyclonedSection = section.cloneNode(true);
           
        
        // Update IDs for the cloned section
        var newId = "mainsection" + clickCounter;
        facultyclonedSection.id = newId;

        // Update IDs for the cloned faculty input
        var facultyInput = facultyclonedSection.getElementsByClassName('faculty-input')[0];
        var facultyInputNewId = 'faculty-input-' + clickCounter;
        facultyInput.id = facultyInputNewId;

           container.appendChild(facultyclonedSection);
           clickCounter++;
   ...
   document.getElementById("classhrbtn").onclick = function () {
    ...
    if (clickCounter2 < 7) {
        ...
        var classhrclonedsection = section.cloneNode(true);
        

// Update IDs for the cloned section
var classhrnewId = "classhrsection" + (clickCounter2 + 1);
classhrclonedsection.id = classhrnewId;
console.log("New classhr section ID: " + classhrnewId);

// Update IDs for the cloned selectday input
var selectdayInput = classhrclonedsection.querySelector('.selectday');
var selectdayInputNewId = 'selectdayinput-' + (clickCounter2 + 1);
selectdayInput.id = selectdayInputNewId;
console.log("New selectday input ID: " + selectdayInputNewId);

        // Increment clickCounter
        clickCounter2++;

        // Append the cloned section to the container
        container.appendChild(classhrclonedsection);

                // Manually reinitialize the dropdowns in the cloned section
                var dropdowns = classhrclonedsection.querySelectorAll('.dropdown-toggle');
        ...
        var clonedDropdown = classhrclonedsection.querySelector('.selectday');
        var clonedDropdownItems = classhrclonedsection.querySelectorAll('.selectday-dropdown-item');
        ...
        clonedDropdownItems.forEach(function (item, index) {
            item.addEventListener('click', function () {
                ...
                // Handle dropdown selection for the cloned section (selectday)
                var clonedSelectday = classhrclonedsection.querySelector('.selectday');
        var clonedSelectdayItems = classhrclonedsection.querySelectorAll('.selectday-dropdown-item');


        // Handle dropdown selection for the cloned section (meettype)
        var clonedMeettype = classhrclonedsection.querySelector('.meettype');
        var clonedMeettypeItems = classhrclonedsection.querySelectorAll('.meettype-dropdown-item');
        ...
        classhrclonedsection.style.position = 'absolute';

        // Set the top position for the FIRST clone
        if (clickCounter2 === 1) {
            classhrclonedsection.style.top = '122px'; // Set the desired top position for the first clone
        } else {
            ...
            classhrclonedsection.style.top = previousCloneBottom + 10 + 'px';
        }
        ...
        classhrclonedsection.appendChild(spanElement);
...
// Display Faculty Data - Gawa ka nalang button para matest
// document.getElementById("displayFacultyButton").onclick = function () {
//     // Create an array to store faculty input values
//     var facultyValues = [];

//     // Iterate through faculty input elements and retrieve their values
//     for (var i = 0; i < clickCounter; i++) {
//         var facultyValue = document.getElementById("faculty-input-" + i).value;
//         facultyValues.push(facultyValue);
//     }

//     // Display the values in order in an alert
//     alert("clickCounter: " + clickCounter + "\nFaculty Input Values: " + facultyValues.join(", "));
// };

// Display Selectday Input IDs
document.getElementById("displaySelectdayIdsButton").onclick = function () {
    var selectdayIds = [];

    // Iterate through each cloned section and get the selectdayInputNewId
    for (var i = 0; i < clickCounter2; i++) {
        var selectdayInputId = document.getElementById('selectdayinput-' + i).id;
        selectdayIds.push(selectdayInputId);
    }

    // Display the array of selectdayInputNewIds in an alert
    alert("Selectday Input IDs: " + selectdayIds.join(', '));
};
